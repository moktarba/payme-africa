...
const DEBUG_PORT = Number(process.env.QA_CHROME_PORT || 9410);
...
async function main() {
  ...
  try {
    ...
    const clickByText = async (needle) => {
      const result = await evaluate(`
        (() => {
          ...
          if (!target) return { ok: false, visible: (document.body.innerText || '').slice(0, 900) };
    ...
    const clickSwitchNearText = async (needle) => {
      const result = await evaluate(`
        (() => {
          const normalize = (value) => String(value || '')
            .normalize('NFKD')
          ...
          const label = [...document.querySelectorAll('*')]
            .filter((element) => normalize(element.innerText || element.textContent) === wanted)
            .sort((a, b) => {
          ...
          for (let i = 0; i < 6; i += 1) {
            if (!row) break;
            const candidates = [...row.querySelectorAll('[role="switch"], input, button, [tabindex]')];
            const target = candidates.find((element) => element !== label);
            if (target) {
              target.dispatchEvent(new MouseEvent('click', { bubbles: true, cancelable: true, view: window }));
              return { ok: true };
            }
          ...
          return { ok: false, reason: 'switch', visible: (document.body.innerText || '').slice(0, 900) };
        })()
      `);
      if (!result?.ok) throw new Error(`Switch introuvable pres de: ${needle}\n${result?.visible || result?.reason || ''}`);
    };
    ...
    await clickByText('Notifications');
    await wait(1500);
    await expectText('2 alertes non lues');
    await expectText('Paiement Wave en attente');
    await expectText('Paiement confirme');
    await expectText('Resume du jour');
    await expectText('Preferences');

    await clickByText('Paiement Wave en attente');
    await wait(800);
    await expectText('1 alertes non lues');

    await clickByText('Tout lire');
    await wait(800);
    await expectText('0 alertes non lues');

    await clickSwitchNearText('Activite equipe');
    await wait(800);
    await expectText('Activite equipe');

    if (browser.errors.length) {
      throw new Error(`Erreurs console detectees:\n${browser.errors.join('\n')}`);
    }

    console.log('QA notifications OK: acces profil -> lecture -> tout lire -> preference');
    browser.ws.close();
  } finally {
    chrome.kill();
  }

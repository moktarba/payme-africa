...
const DEBUG_PORT = Number(process.env.QA_CHROME_PORT || 9370);
...
async function main() {
  ...
  try {
    ...
    await clickByText('Rapports');
    await wait(1500);
    await expectText('Chiffre du jour');
    await expectText('2 500');
    await expectText('Panier moyen');
    await expectText('Repartition par mode');
    await expectText('Articles les plus vendus');

    await clickByText('7 jours');
    await wait(1000);
    await expectText('CA 7 jours');
    await expectText('42 000');
    await expectText('Ventes par jour');

    await clickByText('Ce mois');
    await wait(1000);
    await expectText('Mai 2026');
    await expectText('Evolution par semaine');
    await expectText('180 000');

    if (browser.errors.length) {
      throw new Error(`Erreurs console detectees:\n${browser.errors.join('\n')}`);
    }

    console.log('QA reports OK: acces accueil -> jour -> semaine -> mois');
    browser.ws.close();
  } finally {
    chrome.kill();
  }

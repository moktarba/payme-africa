...
const DEBUG_PORT = Number(process.env.QA_CHROME_PORT || 9390);
...
async function main() {
  ...
  try {
    ...
    const fillByPlaceholder = async (placeholder, value) => {
      const result = await evaluate(`
        (() => {
          ...
          if (!input) return { ok: false, visible: (document.body.innerText || document.body.textContent || '').slice(0, 700) };
    ...
    await clickByText('Catalogue');
    await wait(1500);
    await expectText('Articles rapides');
    await expectText('3 articles rapides disponibles');

    await clickByText('Ajouter');
    await wait(800);
    await fillByPlaceholder('Eau minerale', 'Jus Gingembre');
    await fillByPlaceholder('500', '700');
    await fillByPlaceholder('Boissons', 'Boissons');
    await clickByText('Ajouter article');
    await wait(1200);
    await expectText('Jus Gingembre');
    await expectText('4 articles rapides disponibles');

    await clickByText('Modifier');
    await wait(800);
    await fillByPlaceholder('500', '800');
    await clickByText('Enregistrer');
    await wait(1200);
    await expectText('800');

    await clickByText('Supprimer');
    await wait(1200);
    await expectText('3 articles rapides disponibles');
    await expectNoText('Jus Gingembre');

    if (browser.errors.length) {
      throw new Error(`Erreurs console detectees:\n${browser.errors.join('\n')}`);
    }

    console.log('QA catalog OK: creation -> modification -> suppression article rapide');
    browser.ws.close();
  } finally {
    chrome.kill();
  }
